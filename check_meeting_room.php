...
<html>
    ...
    <body class="hold-transition skin-green sidebar-mini">
        ...
        <div class="wrapper">
            ...
            <?php echo asideMenu('check_meeting_room'); ?>
                ...
                <section class="content">
                    ...
                    <div class="box">
                        <div class="box-header with-border">
                            <h3 class="box-title">รายงานการจองใช้ห้องประชุม</h3>
                        </div>
                        <div class="box-body">
                                                    ...
                                                    <div class="form-group" style="padding-top:25px;">
                                                        ...
                                                        <div class="btn-group">
                                                            ...
                                                            <ul class="dropdown-menu" role="menu">
                                                                <li><a href="check_meeting_room_print.php?start_date=<?php echo $start_date ?>&end_date=<?php echo $end_date ?>&room_id=<?php echo $room_id ?>" target="_blank">Print</a></li>
                                                                <li><a href="check_metting_room_export.php?start_date=<?php echo $start_date ?>&end_date=<?php echo $end_date ?>&room_id=<?php echo $room_id ?>">Export Excel</a></li>
...
if ($room_id == 'all') {
    ...
    $sql = "SELECT events.id,fullname,room_name,title,DATE(start_date) as s_date,DATE(end_date) as e_date,TIME_FORMAT(start_date, '%h:%i') as start_time,TIME_FORMAT(end_date, '%h:%i')  as end_time,detail,person_use,events.status FROM events
	    INNER JOIN room ON room.id = events.room_id 
	    INNER JOIN user ON user.id = events.user_id WHERE DATE(start_date) BETWEEN '$start_date' AND '$end_date' ORDER BY events.start_date DESC";
} else {
    $sql = "SELECT events.id,fullname,room_name,title,DATE(start_date) as s_date,DATE(end_date) as e_date,TIME_FORMAT(start_date, '%h:%i') as start_time,TIME_FORMAT(end_date, '%h:%i')  as end_time,detail,person_use,events.status FROM events
	    INNER JOIN room ON room.id = events.room_id 
	    INNER JOIN user ON user.id = events.user_id  
            WHERE DATE(start_date) BETWEEN '$start_date' AND '$end_date' AND events.room_id = $room_id ORDER BY events.start_date DESC";
}
