<?php
	...
	$end_date = !empty($_GET['end_date']) ? $_GET['end_date'] : date("Y-m-d");
		...
		$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue("G1",'ผู้บันทึกข้อมูล')
			...
			if($room_id == 'all'){
				...
				$sql = "SELECT events.id,fullname,room_name,title,DATE(start_date) as s_date,DATE(end_date) as e_date,TIME_FORMAT(start_date, '%h:%i') as start_time,TIME_FORMAT(end_date, '%h:%i')  as end_time,detail,person_use,events.status FROM events
				INNER JOIN room ON room.id = events.room_id 
				INNER JOIN user ON user.id = events.user_id  WHERE DATE(start_date) BETWEEN '$start_date' AND '$end_date' AND events.room_id = $room_id ORDER BY events.start_date DESC";
			}
